...
add_shortcode( 'invicta_europe_map', 'invicta_europe_map_shortode' );

function invicta_europe_map_shortode( $atts ) {
	...
	if ( $locations ) {
		...
		$countries = invicta_get_countries_europe();
	...
	if ( $dataset ) {
		...
		$guid = 'europe_map_' . uniqid();
	
		$output .= '<div id="' . $guid . '" class="invicta_world_map invicta_europe_map wpb_content_element">';
		
		$output .=		'<div class="map_canvas">';
		$output .=			'<img class="map_graphic" src="' . get_template_directory_uri() . '/styles/images/maps/europe-map.png' . '" alt="" />';
		$output .= 		'</div>';
